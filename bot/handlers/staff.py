...
from services.matcher import CatalogRefreshError, matcher, refresh_catalog
...
STAFF_MENU_TEXTS = {"🔍 Додати товар", "🛒 Кошик", "📤 Надіслати на підтвердження", "📜 Історія", "❓ Допомога", "🔄 Оновити каталог"}
...
🔄 Оновити каталог — підтягнути свіжі товари й техкарти з Poster
❓ Допомога — ця підказка

<i>Каталог оновлюється сам кожні 15 хвилин. Кнопка 🔄 потрібна, лише якщо щойно змінили щось у Poster і хочете побачити це негайно.</i>"""
...
@router.message(F.text == "🔄 Оновити каталог")
async def btn_refresh_catalog(message: Message, state: FSMContext, staff: Staff) -> None:
    await _safe_clear(state)
    async with AsyncSessionLocal() as session:
        client = await session.get(Client, staff.client_id)
    if not client:
        await message.answer("❌ Заклад не знайдено.", reply_markup=staff_menu_kb())
        return

    notice = await message.answer("⏳ Оновлюю каталог з Poster…")
    matcher.invalidate(staff.client_id)
    try:
        count = await refresh_catalog(staff.client_id, client.access_token)
    except CatalogRefreshError:
        await notice.edit_text(
            "❌ Poster зараз не відповідає — каталог лишився попереднім.\n"
            "Спробуйте ще раз за хвилину."
        )
        return
    await notice.edit_text(f"✅ Каталог оновлено — {count} позицій.")
...
async def fsm_menu_interrupt(message: Message, state: FSMContext, staff: Staff) -> None:
    ...
    elif text == "🔄 Оновити каталог":
        await btn_refresh_catalog(message, state, staff)
    elif text == "❓ Допомога":
...
async def handle_search(message: Message, state: FSMContext, staff: Staff) -> None:
    ...
    if matcher.is_stale(client_id):
        ...
        if client:
            try:
                await refresh_catalog(client_id, client.access_token)
            except CatalogRefreshError:
                # Already logged. Search on with the previous catalog rather than
                # failing the whole query — stale results beat no results.
                pass
